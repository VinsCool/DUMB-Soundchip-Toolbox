...
extern bool g_16bit_freq;		//POKEY 16-bit mode 
extern bool g_179mhz_freq;		//POKEY 1.79mHz mode 
extern bool g_15khz_freq;		//POKEY 15kHz mode
...
public:
	...
	double GetCentsOff(int semitone, double pitch, double tuning = 440.0);
	char* GetSemitone(double pitch, double tuning = 440.0);
	int GetOctave(double pitch, double tuning = 440.0);
	int GetNoteNumber(double pitch, double tuning = 440.0);

	double generate_pitch(int soundchip, int freq); 
	int generate_freq(int soundchip, int semitone); 

	double get_pitch(int soundchip, int freq); 
	int get_freq(int soundchip, double pitch); 

	int get_freq_min(int soundchip); 
	int get_freq_max(int soundchip); 

	//POKEY hardcoded functions, need to be re-organised to generic functions instead 
	//double generate_freq(int audc, int audf, int audctl, int channel, int timbre = 0); 
	//int generate_table(int semitone, int timbre, int audctl);
	int get_pokey_audf(double pitch, int coarse_divisor, double divisor, int cycle);
	double get_pokey_pitch(int audf, int coarse_divisor, double divisor, int cycle);
	int delta_pokey_audf(double pitch, int audf, int coarse_divisor, double divisor, int cycle, int timbre); 
